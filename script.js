document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    const getErrorContainer = function (field) {
    ...
    const getErrorMessage = function (field, rule) {
      const customMsg = field.dataset['error' + rule.charAt(0).toUpperCase() + rule.slice(1)];
    ...
    const validateField = function (field) {
      ...
      if (field.hasAttribute('required') && !validators.required(field.value)) {
        ...
        field.value.trim() !== '' &&
        !validators.minlength(field.value, field.getAttribute('minlength'))
      ) {
        isValid = false;
  ...
  const fadeElements = document.querySelectorAll('.' + OBSERVE_CLASS);
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Jump משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'a0689204-eeb4-48b7-9a5c-26f084310738',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'a0689204-eeb4-48b7-9a5c-26f084310738';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'a0689204-eeb4-48b7-9a5c-26f084310738';
